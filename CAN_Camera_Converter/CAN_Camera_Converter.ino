...
void set_id_filters(){
    ...
    mcp2515.setFilterMask(MCP2515::MASK0, false, id_mask); // RXB0
    mcp2515.setFilter(MCP2515::RXF0, false, id_filter);    // RXB0
    mcp2515.setFilterMask(MCP2515::MASK1, false, id_mask); // RXB1
    mcp2515.setFilter(MCP2515::RXF2, false, id_filter);    // RXB1
}
...
void setup(){
    ...
    // Set up MCP2515
    mcp2515.reset();
    MCP2515::ERROR set_bitrate_result = mcp2515.setBitrate(SPEED, CLOCK);
    set_id_filters();
    mcp2515.setNormalMode();
    
    #ifdef SERIAL_DEBUG_ENABLED
        if (set_bitrate_result == MCP2515::ERROR_OK){
            Serial.println(F("Sucessfully set bitrate"));
        }
        else{
            Serial.println(F("Failed to set bitrate. Is the MCP2515 connected?"));
        }
...
void loop(){
    ...
    if (mcp2515.readMessage(&incoming_frame) == MCP2515::ERROR_OK){
        #ifdef SERIAL_DEBUG_ENABLED
            if (show_message){
        ...
        handle_haltech_frame(&incoming_frame, &mcp2515);
    }
